...
function generateLenovoExtraProducts() {
  const families = [
    // Laptops (3 items)
...
function generateAdeshCreationProducts() {
  const families = [
...
function generateGucciProducts() {
  const families = [
...
function generateNoiseProducts() {
  const families = [
...
// Local project images
export function generateSeededProducts() {
  return [
...
export async function ensureSeedData() {
  ...
  const hasOldData = products.length > 0 && products.some(p => p.images && p.images[0] && p.images[0].includes('unsplash.com'));

  if (products.length === 0 || hasOldData) {
    console.log('Bootstrapping default ShopEZ data...');
    if (hasOldData) {
      console.log('Detected old Unsplash product data. Clearing products and reseeding...');
      await ProductRepository.clear();
    }
    ...
    console.log(`Seeded ${seededProducts.length} products for the database.`);
  }
